...
const seasonalCache = new NodeCache({ stdTTL: 18000 });
const seasonalData = new Seasonal(3);
...
export default class extends Listener {
   public async run(interaction: ButtonInteraction, arr: ButtonTuple) {
      try {
         ...
         const buttonRow = seasonalButtonRow(obj.page, obj.authorId, embedData.hasNext);
...
export async function getSeasonalEmbedData(page: number) {
   if (seasonalCache.has(page)) {
      const cacheData = seasonalCache.get<SeasonalData>(page);
      if (!cacheData) throw new Error("variable is missing after check");
      const embedData = seasonalData.getEmbedData(cacheData, page);

      setCacheData(page + 1);

      return embedData;
   } else {
      const data = await seasonalData.getSeasonalData(page);
      seasonalCache.set<SeasonalData>(page, data);
      const embedData = seasonalData.getEmbedData(data, page);
...
export function seasonalButtonRow(page: number, authorId: string, hasNext: boolean) {
...
function setCacheData(page: number) {
   seasonalData.getSeasonalData(page).then((value) => {
      seasonalCache.set<SeasonalData>(page, value);
   });
}
